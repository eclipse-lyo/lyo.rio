...
<%@ page import="org.eclipse.lyo.oslc.rm.services.requirement.Requirement,java.util.*,org.eclipse.lyo.rio.util.*" %>
...
Collection<String> replaces = requirement.getReplaces();
Collection<String> subject = requirement.getSubject();
Collection<String> hasPart = requirement.getHasPart();
XmlFormatter formatter = new XmlFormatter();
formatter.addNamespacePrefix(IRmConstants.OSLC_RM_NAMESPACE,IRmConstants.OSLC_RM_PREFIX); 
String rawRdf = formatter.format(requirement, IRmConstants.OSLC_RM_TYPE_REQUIREMENT);
...
<tr><td colspan="2" align="center"><b>View Definition</b></td></tr>
<tr><td>Is View Of Version:</td><td></td></tr>
<%if( subject.size() == 0 ) { %><tr><td></td><td>(none)</td></tr><%} %>
<% for( String subjectLink : subject ) {
	int pos = subjectLink.lastIndexOf('/');
	String id = subjectLink.substring(pos+1);
%>
<tr><td>
</td><td><a href="<%=subjectLink %>" onmouseover="hover('<%=subjectLink %>','d<%=id %>');" onmouseout="closeHover();"><%=subjectLink %></a>
<div id="d<%=id %>"></div>
</td></tr>
<% } %>
<tr><td>Has Part:</td><td></td></tr>
<%if( hasPart.size() == 0 ) { %><tr><td></td><td>(none)</td></tr><%} %>
<% for( String hasPartLink : hasPart ) {
	int pos = hasPartLink.lastIndexOf('/');
	String id = hasPartLink.substring(pos+1);
%>
<tr><td>
</td><td><a href="<%=hasPartLink %>" onmouseover="hover('<%=hasPartLink %>','d<%=id %>');" onmouseout="closeHover();"><%=hasPartLink %></a>
<div id="d<%=id %>"></div>
</td></tr>
<% } %>
...
<% } %>
<tr><td>Replaces Version:</td><td></td></tr>
<%if( replaces.size() == 0 ) { %><tr><td></td><td>(none)</td></tr><%} %>
<% for( String replacesLink : replaces ) {
	int pos = replacesLink.lastIndexOf('/');
	String id = replacesLink.substring(pos+1);
%>
<tr><td>
</td><td><a href="<%=replacesLink %>" onmouseover="hover('<%=replacesLink %>','d<%=id %>');" onmouseout="closeHover();"><%=replacesLink %></a>
<div id="d<%=id %>"></div>
</td></tr>
<% } %>
...
<tr><td>Nested Classifier (Has View):</td><td><a href="javascript: alert('To be implemented')">Add</a></td></tr>
