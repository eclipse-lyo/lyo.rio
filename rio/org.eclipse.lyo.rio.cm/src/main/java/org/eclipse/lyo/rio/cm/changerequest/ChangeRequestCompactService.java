 ...
 */
public class ChangeRequestCompactService extends RioBaseService {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		RioResource resource;
		try {
			resource = getStore().getResource(uri);
		} catch (RioServerException e) {
			throw new RioServiceException(e);
		}
		if( resource == null ) {
			throw new RioServiceException(IConstants.SC_NOT_FOUND, "ChangeRequest not found");
		}
		ChangeRequest cr = new ChangeRequest(resource.getUri(), resource.getStatements());
		request.setAttribute("changerequest", cr); //$NON-NLS-1$
		String preview = null;
		if( "large".equals(previewType) ) { //$NON-NLS-1$
			preview = "/cm/changerequest_largePreview.jsp"; //$NON-NLS-1$
		} else {
			preview = "/cm/changerequest_smallPreview.jsp"; //$NON-NLS-1$
		}
