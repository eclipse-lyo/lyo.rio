...
public class SetupService extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if( repoPath == null ) {
			repoPath = getServletContext().getRealPath("rio/rio-am/repo"); 
			if( repoPath == null ) {
				// try one last default 
				repoPath = "/rio/rio-am/repo";
			}
		}
		if( binPath == null ) {
			binPath = getServletContext().getRealPath("rio/rio-am/bin");
			if( binPath == null ) {
				binPath = "/rio/rio-am/bin";
			}
		}
