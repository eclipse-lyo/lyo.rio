...
//	private Map<String,Element> idElm = new HashMap<String,Element>();
    ...
    private void setValue(JSONObject jresource,
            String predicate, RioValue value, RioResource resource)
        ...
        if (value.getType() == RioValueType.BLANK_NODE) {
            ...
            List<RioStatement> statements = resource.getStatements(value.stringValue(), IConstants.RDF_TYPE, null);
            if (statements.size() > 0) {
                RioStatement bNodeTypeStatement = statements.get(0);
                RioValue bNodeType = bNodeTypeStatement.getObject();
                if (bNodeType.getType() == RioValueType.URI) {
