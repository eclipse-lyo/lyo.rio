...
public abstract class RioBaseService extends HttpServlet {

	private static final long serialVersionUID = -8870677366709073238L;
	
	protected RioStore getStore() throws RioServiceException {
		try {
			return RioStore.getStore();
	...
	public String getBaseUrl() throws RioServiceException{
		return getStore().getUriBase();
	}
	
	public String getUserUri(String userId) throws RioServiceException {
		if( userId == null ) {
			userId = IConstants.RIO_UNKNOWN_USER_ID;
		}
	...
	public void service(ServletRequest req, ServletResponse res)
		...
		try {
		    ...
		    throw new ServletException(Messages.getString("OriBaseService.NonHttpRequest")); //$NON-NLS-1$
		}
		
		if( !RioStore.isStoreInitialized() ) {
			String dest = request.getRequestURL().toString();
			String url = URLEncoder.encode(dest, IConstants.TEXT_ENCODING);
			RequestDispatcher rd = req.getRequestDispatcher("/setup?url=" + url); //$NON-NLS-1$
			rd.forward(req,res);
			return;
		}
		...
		try{
			...
			RioError error = new RioError(IConstants.SC_INTERNAL_ERROR, sw.toString() );
			response.getWriter().write(error.toString());
		}
	}

	protected void checkConditionalHeaders(HttpServletRequest request, OslcResource resource) throws RioServiceException {
		...
		if( ifMatch == null && ifUnmodifiedSince == null ) {
			throw new RioServiceException(IConstants.SC_BAD, 
