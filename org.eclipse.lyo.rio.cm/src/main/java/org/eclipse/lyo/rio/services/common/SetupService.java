...
public class SetupService extends HttpServlet {
	...
	private static final long serialVersionUID = -4041330662058504323L;
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if( host != null && repoPath != null && binPath != null && url != null ) {
			...
			try {
				RioStore.initalizeStore(repoPath, binPath, host, ICmConstants.SERVER_CONTEXT);
		...
		if( repoPath == null ) {
			repoPath = getServletContext().getRealPath("rio/rio-cm/repo"); 
			if( repoPath == null ) {
				// try one last default 
				repoPath = "/rio/rio-cm/repo";
			}
		}
		if( binPath == null ) {
			binPath = getServletContext().getRealPath("rio/rio-cm/bin");
			if( binPath == null ) {
				binPath = "/rio/rio-cm/bin";
			}
		}
	...
	/**
	 * Get ServletContext's realPath, falling back to 'rio.realpath' property if not available.
	 */
	protected String getRealPath(String path) {
		String realpath = getServletContext().getRealPath(path); //$NON-NLS-1$
		if (realpath == null) {
			realpath = System.getProperty("realpath") + "/" + path;
		}
		return realpath;
	}
}
